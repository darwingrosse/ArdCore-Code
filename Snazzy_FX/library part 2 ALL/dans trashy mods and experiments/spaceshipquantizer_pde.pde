...
void loop()
{
  ...
  if (inValue != testValue) {
    testValue = inValue;
    int newValue = quantNote(inValue, analogRead(0));
  ...
  if (doQuant) {
    // send the note
    outValue = quantNote(inValue,analogRead(0));
    dacOutput(outValue);
...
//  weirdNote(int) - drop an incoming value to a note value
//  -------------------------------------------------------
int quantNote(int v, int x)
{
  int tempVal = v >> 4;  // decrease the value to 0-64 - ~ a 5 volt range
  tempVal *= transpose;  // add the transposition
   return ((tempVal << 4)*sin(x))
  ;
  
   
  
 // increase it to the full 8-bit range
}
